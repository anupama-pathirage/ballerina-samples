function main (string[] args) {
    ...
    int ret = testDB.update("CREATE TABLE ArrayTypesTest(row_id INTEGER,int_array INTEGER ARRAY, long_array BIGINT ARRAY,
			float_array FLOAT ARRAY, double_array DOUBLE ARRAY, boolean_array BOOLEAN ARRAY,
			string_array  VARCHAR(50) ARRAY)", null);
    ...
    while (dt.hasNext()) {
        ...
        println(int_arr.length() + "|" + long_arr.length() + "|" + float_arr.length() + "|" + boolean_arr.length() + "|" + string_arr.length());
    }
    testDB.close();
}
